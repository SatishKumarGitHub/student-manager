...
def add_student(table_name,df_students):
    ...
    for row,index in df_students.iterrows():
        lst_row ='',''.join(row.tolist())
        _c.execute("insert into " + table_name + "(" + columns + ")  values('" + lst_row + "')")
...
def read_student_from_db(id,limit=10):
    _conn = sqlite3.connect(student_db_file_location)
    _c = _conn.cursor()
    command = "SELECT * FROM Student WHERE  1=1 " 
    if (id is not None):
        command+=" and RollNo LIKE '%" + id.upper() +"%'"
...
def match_user_id_with_student_id(student_id):
    # Given the note id, confirm if the current user is the owner of the note which is being operated.
    _conn = sqlite3.connect(note_db_file_location)
    _c = _conn.cursor()

    command = "SELECT user FROM Student WHERE RollNo = '" + student_id + "';" 
    _c.execute(command)
    result = _c.fetchone()[0]
...
def searh_student_name(student_name,student_dept):
    # Given the note id, confirm if the current user is the owner of the note which is being operated.
    _conn = sqlite3.connect(note_db_file_location)
    _c = _conn.cursor()
    command = "SELECT user FROM Student WHERE  1=1 " 
    if (student_name  is not None):
        command+=" and upper(Name) like '%" + student_name.upper() +"%'"
    if (student_dept  is not None):
        command+=" and upper(Department)='" + student_dept.upper() +"'"
    _c.execute(command)
    result = _c.fetchall()
...
def delete_student_from_db(student_id):
    ...
    command = "DELETE FROM Student WHERE RollNo = '" + student_id + "';" 
    _c.execute(command)
