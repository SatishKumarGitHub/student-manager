...
from database import read_student_from_db, match_user_id_with_student_id, searh_student_name, delete_student_from_db
...
def FUN_private():
    if "current_user" in session.keys():
        ...
        notes_table = zip([x[0] for x in notes_list],\
                          ...
                          ["/delete_notes/" + x[0] for x in notes_list])
...
def FUN_student():
    if "current_user" in session.keys():
        student_list = read_student_from_db(None,10)
        student_table = zip([x[0] for x in student_list],\
                          [x[1] for x in student_list],\
                          [x[2] for x in student_list],\
                          [x[3] for x in student_list],\
                          ["/delete_student/" + x[0] for x in student_list])

               

        return render_template("student_page.html", students = student_table)
    else:
        return abort(401)
